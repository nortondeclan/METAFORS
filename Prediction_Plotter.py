...
run_label = "sample_lorenz_predictions"

#method = "async_sm" #metafors, learning forecaster model parameters but no cold-start vectors
...
noisy_targets = False
fixed_reg = True
return_dims = [0, 1, 2] #Include all components of the Lorenz state (x,y,z) in the long and short time series
#return_dims = [2] #Include only the z-component of the Lorenz state in the long and short time series
...
lib_length = 5000
val_length = 3000
test_lengths = [30] #[150] #[200]
noise_amp = None

sigma_rng = [7.5, 12.5]
omega_rng = [.75, 1.25]
val_sigma_rng = [7.5, 12.5]
val_omega_rng = [.75, 1.25]
...
if grid_val:
    ...
    test_library.generate_grid(
        ranges = [val_sigma_rng + [int(np.sqrt(val_size))], val_omega_rng + [int(np.sqrt(val_size))]],
        seed = val_seed
        )    
else:
...
def train_and_get_predictions(
    ...
    if method == "one_step_sm_ri":
        predictions = get_one_step_predictions(
            seed = pred_esn_args["seed"],
            test_length = test_length,
            incl_ri = True
            )
...
if use_parallel:
    jlib.Parallel(n_jobs = -1)(jlib.delayed(
        predictions = train_and_get_predictions)(
            method = method,
            test_length = test_length,
...
with mpl.rc_context({'font.size': font_size}):
    for prediction in predictions:
        fig, axs = plt.subplots(len(return_dims), 1,
                                figsize = (10, 2 * len(return_dims)), 
        ...
        for i, ax in enumerate(axs):
            ...
            if i == 0:
                ...
                label = None
            ax.axvline(x = prediction.resync_inputs.shape[0] - 1,
                       linestyle = "--", color = "k",
                       label = label)
            
            if i == 0:
                label = "End of Valid Prediction"
            else:
                label = None
            ax.axvline(x = prediction.valid_length() + prediction.resync_inputs.shape[0],
                       linestyle = "--",
                       color = "r",
                       label = label
                       )
            
            ax.set_ylabel(f"$x_{return_dims[i]+1}$")
            if prediction.reservoir_outputs.shape[0] > max_horizon:
                ax.set_xlim(right = max_horizon, left = -30)
        fig.legend(loc = "outside upper center",
                   ncols = 4,
                   frameon = frame_legend)
        axs[-1].set_xlabel("Time (Time Steps, $\\Delta t$)")

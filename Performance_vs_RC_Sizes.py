...
equal_sizes_only = False #True
seeds = [int(i + 1) for i in np.arange(10, dtype = int)]
test_length = 200

if fig_s2A:
    run_label = "paper_performance_vs_rc_sizes"
    return_dims = [0, 1, 2]
    mapper_sizes = [1000]
    pred_sizes = [50, 100, 250, 500, 600, 700, 800, 900, 1000, 1250, 1500, 1750, 2000, 2250, 2500, 3000, 4000, 5000]
elif fig_s2B:
    run_label = "paper_performance_vs_rc_sizes"
    return_dims = [0, 1, 2]
    pred_sizes = [500]
    mapper_sizes = [50, 100, 250, 500, 600, 700, 800, 900, 1000, 1250, 1500, 1750, 2000, 2250, 2500, 3000, 4000, 5000]
elif fig_s2C:
    run_label = "paper_performance_vs_rc_sizes_z_only"
    return_dims = [2]
    mapper_sizes = [1000]
    pred_sizes = [50, 100, 250, 500, 600, 700, 800, 900, 1000, 1250, 1500, 1750, 2000, 2250, 2500, 3000, 4000, 5000]
elif fig_s2D:
...
method_labels = {
    "async_sm" : "Unsync_SM_W",
    "async_sm_ri" : "Unsync_SM_riW",
    "library_interpolation" : "library_interpolation",
    "nearest_euclidean" : "nearest_euclidean",
    "library_average" : "library_average",
    "vanilla" : "vanilla",
    "long_vanilla" : "long_vanilla",
    "batch" : "batch",
    }
...
def train_and_get_predictions(
    ...
    if same_RCs:
        ...
        map_esn_args = {
            ...
            "input_dimension" : len(return_dims),
            "connections" : 3,
            "seed" : seed + len(seeds)**2
            }
        
    run_directory = join(os.getcwd(), run_label)
    check_loc = join(run_directory, method_labels[method]) #"Unsync_SM_riW")
    check_loc = join(check_loc, str(pred_esn_args["seed"]))
    check_loc = join(check_loc, file_name + ".pickle")
    
    if os.path.exists(check_loc) and skip_repeats:
        print("Data Already Exists in Folder: ", file_name)
    else:
        print("Starting, Folder: ", file_name)
    ...
    if use_parallel:
        jlib.Parallel(n_jobs = -1)(jlib.delayed(
            train_and_get_predictions)(
                method = method,
                test_length = test_length,
                pred_size = mapper_size,
                mapper_size = mapper_size,
                seed = seed,
                file_name = join(str(mapper_size), str(mapper_size)),
                fixed_reg = fixed_reg,
                pred_reg = pred_regs[method],
                mapper_reg = mapper_regs[method],
                async_sample_separation = async_sample_separation
                )
                for mapper_size in mapper_sizes
                for seed in seeds
                )
    else:
        for mapper_size in mapper_sizes:
    ...
    if use_parallel:
        jlib.Parallel(n_jobs = -1)(jlib.delayed(
            train_and_get_predictions)(
                ...
                mapper_size = mapper_size,
                seed = seed,
                file_name = join(str(mapper_size), str(pred_size)),
                fixed_reg = fixed_reg,
                pred_reg = pred_regs[method],
                mapper_reg = mapper_regs[method],
                async_sample_separation = async_sample_separation

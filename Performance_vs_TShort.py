# Generate data for Fig.s 6C, 6D, S4, and S5 
...
fig_6C = False #True
fig_6D_S4 = True #False
fig_S5A = False
fig_S5B = False

#method = "async_sm" #metafors, learn forecaster model parameters but no cold-start vectors
method = "async_sm_ri" #metafors, full
#method = "library_interpolation"
#method = "nearest_euclidean"
#method = "library_average"
#method = "vanilla"
#method = "batch" #multitask
#method = "one_step_sm_ri"

# For fig 6D, S4, and S5B:

#'METAFORS, Cold-start Only'
#method = "async_sm_ri_no_W" #metafors, learn cold-start vectors but no forecaster parameters
use_state_matcher = False
use_extrapolation_function = False
...
#Baseline 'Search Training Data'
'''
method = "long_vanilla"
use_state_matcher = True
use_extrapolation_function = False
'''

#Baseline 'Back-extrapolate as a constant'
'''
method = "long_vanilla"
use_state_matcher = False
use_extrapolation_function = True
'''

if fig_6C:
    return_dims = [2]
    run_label = "performance_vs_test_length_z_only"
    sigma_rng = [7.5, 12.5]
    omega_rng = [.75, 1.25]
    val_sigma_rng = [7., 13.]
    val_omega_rng = [.7, 1.3]
    ...
    return_dims = [2]
    run_label = "performance_vs_test_length_ri_noW_z_only"
    sigma_rng = [10., 10.]
    omega_rng = [1., 1.]
    val_sigma_rng = sigma_rng
    val_omega_rng = omega_rng
    lib_size = 1
elif fig_S5A:
    return_dims = [0, 1, 2]
    run_label = "performance_vs_test_length"
    sigma_rng = [7.5, 12.5]
    omega_rng = [.75, 1.25]
    val_sigma_rng = [7., 13.]
    val_omega_rng = [.7, 1.3]
    ...
    return_dims = [0, 1, 2]
    run_label = "performance_vs_test_length_ri_noW"
    sigma_rng = [10., 10.]
    omega_rng = [1., 1.]
    val_sigma_rng = sigma_rng
    val_omega_rng = omega_rng
...
noise_amp = 1e-7
if method == "vanilla":
    test_lengths = [2,3,5,7,10,12,15,20,25,30,40,50,60,70,80,90,100,125,150] #Need at least 2 data points in the test signal to train directly on the test signal.
else:
    test_lengths = [1,2,3,5,7,10,12,15,20,25,30,40,50,60,70,80,90,100,125,150]
...
def train_and_get_predictions(
    ...
    if use_state_matcher:
        if not same_RCs:
            map_esn_args["seed"] = 10000 - pred_esn_args["seed"]
            
        state_matcher = rch.get_reservoir_state_matcher(
            lookup_signals = [signal[transient_length:] for signal in train_library.data],
            lookup_states = [rc.ESN(**map_esn_args).predict(
                train_result = np.zeros((
                    mapper_feature.feature_size(map_esn_args["size"], map_esn_args["input_dimension"]),
                    1)),
                inputs = signal, initial_state = np.zeros(map_esn_args["size"]),
                mapper = rch.drive_mapper, feature_function = mapper_feature
                ).reservoir_states[transient_length:] for signal in train_library.data]
            )
    else:
        state_matcher = None
    ...
    def save_predictions(
        ...
        experimental_parameters = {
            ...
            #"extrapolation_function" : extrapolation_function
            }
        optimization_info = {}
    ...
    def get_one_step_predictions(seed: int, test_length: int,
                                 incl_ri: bool = False, incl_rf: bool = False):
        
        return rch.One_Step_SM_Train_and_Predict(
            seed = seed,
            run_label = run_label,
            file_name = file_name,
            pred_esn_args = pred_esn_args,
    ...
    if method == "one_step_sm_ri":
        get_one_step_predictions(
            seed = pred_esn_args["seed"],
            test_length = test_length,
            incl_ri = True
            )
        
    if method == "async_sm_ri_no_W":
        
        vanilla_weights = rc.ESN(**pred_esn_args).train(
            inputs = train_library.data,
            transient_length = transient_length,
        ...
        get_async_predictions(
            ...
            test_length = test_length,
            incl_ri = True,
            fixed_pred_weights = vanilla_weights
            )
    ...
    if method == "long_vanilla":
        ...
        if use_state_matcher:                
            state_matcher = rch.get_reservoir_state_matcher(
                lookup_signals = [signal[transient_length:] for signal in train_library.data],
                lookup_states = [rc.ESN(**pred_esn_args).predict(
                    train_result = np.zeros((
                        pred_feature.feature_size(pred_esn_args["size"], pred_esn_args["input_dimension"]),
                        1)),
                    inputs = signal, initial_state = np.zeros(pred_esn_args["size"]),
                    mapper = rch.drive_mapper, feature_function = pred_feature
                    ).reservoir_states[transient_length:] for signal in train_library.data]
                )
            label += "_state_matching"
            extrap_func = None
        elif use_extrapolation_function:
            label += "_extrap_" + back_extrap_type
            extrap_func = extrapolation_function
            state_matcher = None
        else:
            state_matcher = None
            extrap_func = None
